...
MIN_INPUT_LATENCY_S = 0.0
MAX_INPUT_LATENCY_S = 0.75
...
class PlayerProfile:
    ...
    def _load(self) -> dict:
        ...
        try:
            ...
            normalized["input_latency_s"] = self._normalize_input_latency(
                normalized.get("input_latency_s", DEFAULT_SETTINGS["input_latency_s"])
            )
            normalized.pop("reaction_time_s", None)
    ...
    def _normalize_input_latency(self, value: object) -> float:
        try:
            parsed = float(value)
        except (TypeError, ValueError):
            parsed = float(DEFAULT_SETTINGS["input_latency_s"])
        return max(MIN_INPUT_LATENCY_S, min(MAX_INPUT_LATENCY_S, parsed))

    def record_latency_calibration(self, latency_s: float) -> float:
        calibrated = self._normalize_input_latency(latency_s)
        settings = self.settings()
        settings["input_latency_s"] = calibrated
        self.data["settings"] = settings
        history = self.data.get("calibration_history")
        if not isinstance(history, list):
            history = []
        history.append(
            {
                "input_latency_ms": int(round(calibrated * 1000.0)),
                "recorded_at": datetime.now(timezone.utc).isoformat(),
            }
        )
        self.data["calibration_history"] = history[-20:]
        self.save()
        return calibrated
    ...
    def update_settings(
        ...
        if input_latency_s is not None:
            settings["input_latency_s"] = self._normalize_input_latency(input_latency_s)
